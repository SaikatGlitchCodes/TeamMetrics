 ...
 */
function computeSLAByMonth(prs) {
  if (!prs || prs.length === 0) return [];

  const monthlyGroups = {};

  prs.forEach((pr) => {
    if (!pr.estimated_end_date) return;
    ...
    const firstReview = pr.first_reviewer_submitted_at
      ? new Date(pr.first_reviewer_submitted_at)
      : pr.first_approval_platform
...
// ─── Main Component ───────────────────────────────────────────────────────────
export function SLAChart({ teamId, quarter, year }) {
  ...
  const slaData = computeSLAByMonth(allPRs);
              ...
              {slaData.map((m, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="text-right py-2 px-2 font-semibold text-rose-600">
                      {m.outsideSLA}
                    </td>
